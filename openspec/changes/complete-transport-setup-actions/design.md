...
- Диалог открывается из двух мест: (1) постоянная кнопка "Edit profile" в `WorkbenchCommandBar` — доступна независимо от состояния транскрипта/сессии; (2) компактная карточка "Configure connection" в `AcpConnectionScreen` для пустого/disconnected состояния.
