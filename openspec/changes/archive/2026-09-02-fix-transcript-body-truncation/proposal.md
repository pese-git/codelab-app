...
- `agent-workbench-ui`: уточняется существующий сценарий "Меняется режим отображения" требования "Паттерны взаимодействия agent workbench" — добавляется явный сценарий о том, что содержательный текст записи (реплика пользователя, ответ агента, причина approval, диагностика) рендерится полностью в любом `viewMode`, а варьируется только детализация tool call. Само требование "не теряя данных" не меняется по сути — это исправление реализации под уже одобренную спецификацию, но добавленный сценарий закрепляет это явно и даёт tooling'у delta-спеку для сверки.
