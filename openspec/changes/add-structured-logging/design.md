...
### 7. Глобальный singleton пакета (`getLogger()`/`StructlogConfiguration.configure()`) — сознательное исключение из §23/§34

Пакет по умолчанию работает через process-global mutable state: `StructlogConfiguration.current`/`.configure()`/`.setSinkEnabled()` (static) и `getLogger()` (top-level функция, читающая `.current`). Это и есть идиоматичный способ использования `structured_log` — так его использует собственный `example/main.dart` пакета, explicit-instance путь (`BoundLogger(config, ...)` напрямую) там не демонстрируется вообще.

**Решение:** использовать этот путь как есть. `CodeLabLoggingModule` вызывает `StructlogConfiguration.configure(processors: ..., sinks: ...)` один раз при bootstrap приложения; наш Logger-порт (§33 `observability.md`) — тонкая обёртка над `getLogger([name])`, без собственного хранимого `StructlogConfiguration`-инстанса. Runtime-toggle protocol tracing (`setSinkEnabled('protocol', ...)`) — тоже глобальная операция, что соответствует продуктовому смыслу: единственное debug-переключение на процесс, а не per-session state.

Это сознательное отступление от общего правила §23 `layers-and-dependencies.md` ("DI через composition root, не global service locator") и буквального прочтения §34 `observability.md` ("Logger DI: если logger является dependency, его следует передавать через DI/composition root") — по аналогии с уже существующим в AGENTS.md §10 паттерном явно помеченных исключений (там — `fs/*`/`terminal/*` permission-гейт). Обоснование: логгер — cross-cutting infrastructure concern, а не business dependency; классическая критика service locator ("скрывает зависимости, мешает unit-тестировать бизнес-логику в изоляции") к нему почти не применима — присутствие/отсутствие логов не влияет на корректность тестируемой бизнес-логики. Продевать Logger через constructor injection в `acp_protocol` (pure Dart, без DI framework) также неудобно — там порт логгера в любом случае свёлся бы к top-level accessor, а не к injected-зависимости.

**Остаточный риск:** несколько `AcpClientApplication` в одном тестовом файле, созданных без сброса между тестами, разделят один `StructlogConfiguration.current`. `package:test` по умолчанию изолирует тестовые *файлы* отдельными VM isolates, поэтому межфайловой утечки state не возникает — риск остаётся только внутри одного файла и закрывается явным `StructlogConfiguration.reset()`/`.configure(...)` в `setUp`/`tearDown` (тот же паттерн, что и в `example/main.dart` пакета между демонстрационными сценариями).

Альтернатива (отклонена; была предыдущей версией этого решения): держать explicit `StructlogConfiguration`-инстанс и передавать его напрямую в `BoundLogger(config, ...)`, минуя `getLogger()`. Технически поддерживается конструктором пакета, но добавляет DI-ceremony ради изоляции, которая для логгера не критична и по большей части уже обеспечивается бесплатно test-isolate-per-file поведением `package:test`.

**Нюанс с `initialContext`, подтверждённый по коду 0.2.0-dev.4:** сидирование `StructlogConfiguration.initialContext` в context реализовано именно внутри `getLogger()` (`context = Map.from(config.initialContext)` перед созданием `BoundLogger`) — то есть при использовании `getLogger()` (это решение) `initialContext`, если когда-нибудь понадобится (например, `app_version`/`platform` на каждой записи, §51 `observability.md`), будет применяться автоматически, без дополнительного кода с нашей стороны.
